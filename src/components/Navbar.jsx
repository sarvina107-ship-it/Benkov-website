    ...
    <header className="mt-[20px]">
      ...
      <div className="flex items-center justify-between mx-[60px] mb-5 max-lg:mx-6 max-md:mx-4 max-sm:flex-col max-sm:gap-4">
        ...
        <Link to={ROUTES.CONDITIONS}>
          <button className="group relative px-10 py-4 rounded-xl font-bold uppercase tracking-[0.2em] text-sm bg-[var(--gold-primary)] text-[#1B2A44] dark:text-gray-900 border-2 border-[var(--gold-primary)] hover:bg-transparent hover:text-[var(--gold-primary)] dark:hover:text-[var(--gold-primary)] transition-colors duration-300 overflow-hidden">
            <span className="absolute top-0 left-0 w-2 h-[2px] bg-[#1B2A44] dark:bg-gray-900 group-hover:bg-[var(--gold-primary)] group-hover:w-full transition-all duration-500"></span>
            <span className="absolute bottom-0 right-0 w-2 h-[2px] bg-[#1B2A44] dark:bg-gray-900 group-hover:bg-[var(--gold-primary)] group-hover:w-full transition-all duration-500"></span>
            <span className="relative z-10 flex items-center justify-center gap-3">
              <span className="h-[1px] w-0 bg-[var(--gold-primary)] group-hover:w-4 transition-all duration-500"></span>
              {t('btn_apply')}
              <span className="h-[1px] w-0 bg-[var(--gold-primary)] group-hover:w-4 transition-all duration-500"></span>
            </span>
      ...
      {/* Меню навигации */}
      <div className="bg-[#1B2A44] dark:bg-gray-900 relative z-50">
        {/* ДЕСКТОПНАЯ НАВИГАЦИЯ - показывается на lg и выше */}
        <div className="hidden lg:block">
          <nav className="max-w-7xl h-[70px] mx-auto flex items-center px-6 text-white dark:text-gray-200">
            <div className="flex items-center gap-10">
              <Link to={ROUTES.HOME} className="text-[16px] font-semibold hover:text-[var(--gold-primary)] transition whitespace-nowrap">{t('nav.home')}</Link>

              <div className="relative group">
                <button className="text-[16px] font-semibold hover:text-[var(--gold-primary)] transition flex items-center gap-1 whitespace-nowrap">
              ...
              <div className="relative group">
                ...
                <div className="absolute top-full left-0 mt-0 pt-3 w-[220px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity duration-300 z-50">
                  <div className="bg-white dark:bg-gray-800 text-[#1B2A44] dark:text-gray-200 rounded-[14px] shadow-xl overflow-hidden">
                    <Link to={ROUTES.CONDITIONS} className="block px-5 py-3 hover:bg-[#F5EFE6] dark:hover:bg-gray-700 whitespace-nowrap">{t('nav.conditions')}</Link>
                    <Link to={ROUTES.DOCUMENTS} className="block px-5 py-3 hover:bg-[#F5EFE6] dark:hover:bg-gray-700 whitespace-nowrap">{t('nav.documents')}</Link>
                    <Link to={ROUTES.CONTACTS} className="block px-5 py-3 hover:bg-[#F5EFE6] dark:hover:bg-gray-700 whitespace-nowrap">{t('nav.contacts')}</Link>
                  </div>
                </div>
              </div>

              <Link to={ROUTES.NEWSLIST} className="text-[16px] font-semibold hover:text-[var(--gold-primary)] transition whitespace-nowrap">{t('nav.news')}</Link>
              <Link to={ROUTES.GALLERY} className="text-[16px] font-semibold hover:text-[var(--gold-primary)] transition whitespace-nowrap">{t('nav.gallery')}</Link>
            </div>

            {/* Правая часть: поиск, тема и языки */}
        ...
        {/* Мобильный поиск */}
        {mobileSearchOpen && (
          <div className="lg:hidden px-4 py-3 bg-[#1B2A44] dark:bg-gray-900 border-t border-white/10 dark:border-gray-800">
        ...
        {mobileMenuOpen && (
          <>
            <div className="fixed inset-0 bg-black/50 dark:bg-black/70 z-[100] lg:hidden" onClick={() => setMobileMenuOpen(false)} />
            <div ref={mobileMenuRef} className="fixed top-0 right-0 h-full w-[300px] bg-[#1B2A44] dark:bg-gray-900 shadow-2xl z-[101] lg:hidden overflow-y-auto">
              <div className="p-5">
                ...
                <div className="flex flex-col space-y-2">
                  <Link to={ROUTES.HOME} onClick={() => setMobileMenuOpen(false)} className="text-white dark:text-gray-200 py-3 px-4 hover:bg-white/10 dark:hover:bg-gray-800 rounded-xl transition">{t('nav.home')}</Link>

                  {/* О школе - мобильный dropdown */}
                  <div>
                    <button onClick={() => toggleMobileDropdown('about')} className="text-white dark:text-gray-200 py-3 px-4 hover:bg-white/10 dark:hover:bg-gray-800 rounded-xl transition flex items-center justify-between w-full">
                  ...
                  <div>
                    <button onClick={() => toggleMobileDropdown('education')} className="text-white dark:text-gray-200 py-3 px-4 hover:bg-white/10 dark:hover:bg-gray-800 rounded-xl transition flex items-center justify-between w-full">
                  ...
                  <div>
                    <button onClick={() => toggleMobileDropdown('admission')} className="text-white dark:text-gray-200 py-3 px-4 hover:bg-white/10 dark:hover:bg-gray-800 rounded-xl transition flex items-center justify-between w-full">
                    ...
                    {activeMobileDropdown === 'admission' && (
                      <div className="pl-6 pb-2 space-y-1">
                        <Link to={ROUTES.CONDITIONS} onClick={() => { setMobileMenuOpen(false); setActiveMobileDropdown(null); }} className="block text-white/80 dark:text-gray-300 py-2 px-4 text-sm hover:text-[var(--gold-primary)] transition rounded-lg">{t('nav.conditions')}</Link>
                        <Link to={ROUTES.DOCUMENTS} onClick={() => { setMobileMenuOpen(false); setActiveMobileDropdown(null); }} className="block text-white/80 dark:text-gray-300 py-2 px-4 text-sm hover:text-[var(--gold-primary)] transition rounded-lg">{t('nav.documents')}</Link>
                        <Link to={ROUTES.CONTACTS} onClick={() => { setMobileMenuOpen(false); setActiveMobileDropdown(null); }} className="block text-white/80 dark:text-gray-300 py-2 px-4 text-sm hover:text-[var(--gold-primary)] transition rounded-lg">{t('nav.contacts')}</Link>
                      </div>
                    )}
                  </div>

                  <Link to={ROUTES.NEWSLIST} onClick={() => setMobileMenuOpen(false)} className="text-white dark:text-gray-200 py-3 px-4 hover:bg-white/10 dark:hover:bg-gray-800 rounded-xl transition">{t('nav.news')}</Link>
                  <Link to={ROUTES.GALLERY} onClick={() => setMobileMenuOpen(false)} className="text-white dark:text-gray-200 py-3 px-4 hover:bg-white/10 dark:hover:bg-gray-800 rounded-xl transition">{t('nav.gallery')}</Link>
                </div>
              </div>
            </div>
          </>
      ...
      <section className={`relative ${isHome ? 'h-[85vh] max-lg:h-[70vh] max-md:h-[60vh]' : 'h-[40vh]'} overflow-hidden`}>
        <div className="absolute -top-[1px] inset-0 z-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#1B2A44] dark:from-gray-900 via-transparent to-transparent opacity-80"></div>
        </div>
            ...
            <div className="relative z-10 h-full flex items-center px-[30px] max-lg:px-4">
              <div className="max-w-[750px] max-lg:text-center">
                <h1
                  ...
                  style={{ fontFamily: "'Playfair Display', serif" }}>
                  {t('hero.welcome')}<br />
                  <span className="text-[var(--gold-primary)]">{t('hero.school_name')}</span>
                </h1>
                ...
                <div className="mt-10 flex gap-5 max-md:flex-col max-md:items-center">
                  <Link to={ROUTES.ABOUT} className="max-md:w-full">
                    <button className="group relative px-10 py-4 max-md:px-6 max-md:py-3 bg-[#1B2A44] dark:bg-gray-800 text-white rounded-xl font-bold uppercase tracking-[0.2em] text-sm border border-[var(--gold-primary)]/30 transition-colors duration-300 hover:border-white w-full">
                  ...
                  <Link to={ROUTES.CONDITIONS} className="max-md:w-full">
                    <button className="group relative px-10 py-4 max-md:px-6 max-md:py-3 rounded-xl font-bold uppercase tracking-[0.2em] text-sm bg-[var(--gold-primary)] text-[#1B2A44] dark:text-gray-900 border-2 border-[var(--gold-primary)] hover:bg-transparent hover:text-[var(--gold-primary)] transition-colors duration-300 overflow-hidden w-full">
                      <span className="absolute top-0 left-0 w-2 h-[2px] bg-[#1B2A44] dark:bg-gray-900 group-hover:bg-[var(--gold-primary)] group-hover:w-full transition-all duration-500"></span>
                      <span className="absolute bottom-0 right-0 w-2 h-[2px] bg-[#1B2A44] dark:bg-gray-900 group-hover:bg-[var(--gold-primary)] group-hover:w-full transition-all duration-500"></span>
                      <span className="relative z-10 flex items-center justify-center gap-3">
                        <span className="h-[1px] w-0 bg-[var(--gold-primary)] group-hover:w-4 transition-all duration-500"></span>
